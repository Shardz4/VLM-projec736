...
class SpatialEvaluator:
    ...
    @torch.no_grad()
    def evaluate_batch(self, images: torch.Tensor, subjects: List[str], relations: List[str], objects: List[str], labels: Optional[torch.Tensor] = None) -> List[Dict[str, Any]]:
        ...
        text_feats = self.encoder.encode_texts(flat_prompts)
    ...
    def evaluate_dataset(self, dataloader: DataLoader, only_positive_triplets: bool = True) -> Dict[str, Any]:
        ...
        per_relation = defaultdict(lambda: {"correct": 0, "total": 0, "margins": []})
        ...
        for batch in tqdm(dataloader, desc="Spatial Probe", unit="batch"):
            ...
            batch_res = self.evaluate_batch(images, subjects, relations, objects, labels)
            for item in batch_res:
                rel = item["relation"]
                per_relation[rel]["total"] += 1
                per_relation[rel]["correct"] += int(item["is_correct"])
                per_relation[rel]["margins"].append(item["margin"])
